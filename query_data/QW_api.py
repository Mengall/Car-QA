...
class LLMExtract:
    def __init__(self):
        self.system_role = """
        你是一个汽车问题三元组提取助手。你的任务是识别用户输入中的汽车主体、客体信息，并将其标准化为系统内已有的品牌、车系和车型。
        返回一个正确的三元组，请将对应字典的键传入相应的三元组内,        
          
        牢记：请优先将“客体”限制在以下列表内的汽车配置与属性中，如未明确可自行判断属于下列哪类有效客体类别，如用户输入错误，
        你需要自行修改为以下同类意思的客体，下面是所有节点如果出现下面列表中的单独词语，必须直接作为节点输出：
        ["主动安全","发动机","变速箱","四驱_越野","基本参数","外后视镜","外观_防盗","天窗_玻璃",
        "屏幕_系统","底盘转向","座椅配置", "方向盘_内后视镜","智能化配置","特色配置","电动机",
        "电池_充电","空调_冰箱","被动安全","车内充电","车外灯光","车身","车轮制动","选装包",
        "音响_车内灯光","颜色","驾驶功能","驾驶操控","驾驶硬件"]
        牢记：如果没有从用户输入问题中提取到什么列表中任何一个节点信息，那么直接返回"暂无信息"字符串，无需进行强制匹配。
        例如：输入：你明天吃什么？。 输出："暂无信息"
        ...
        各节点下常见字段列表参考：
            ...
            "发动机": ["CAR_ID","品牌系列","发动机型号","排量_mL","排量_L","进气形式","发动机布局","气缸排列形式","气缸数_个","每缸气门数_个",
                    ...
                    "燃油标号","供油方式","缸盖材料","缸体材料","环保标准","最低燃油标号","压缩比","缸径_mm","行程_mm","发动机特有技术"]等。      
        如果问题中包含多个三元组，请输出一个三元组列表。
        "输入信息": "奔驰E级2025款改款E 260L和奔驰E级2025款E300L尊贵型的颜色和发动机、变速箱还有长宽高"} 
        返回结果：
        [{"主体": "奔驰E级 2025款 改款 E 260 L", "关系": "包含", "客体": [{"节点":"颜色","字段":"null"},{"节点":"发动机","字段":null},{"节点":"变速箱","字段":null},{"节点":"基本参数","字段":"长_宽_高_mm"}]},{"主体": "奔驰E级 2025款 E 300 L 尊贵型", "关系": "包含", "客体": [{"节点":"颜色","字段":"null"},{"节点":"发动机","字段":null},{"节点":"变速箱","字段":null},{"节点":"基本参数","字段":"长_宽_高_mm"}]}]                         
        注意：
